...
use AdminBundle\Form\GoodsType;
use AdminBundle\Util\Files\DirectoryUtil;
...
class GoodController extends Controller{
     ...
     * @Route("/", name="good")
     * @Method("GET")
     * @Template("AdminBundle:Good:index.html.twig")
     * @return array
     */
    ...
    public function indexAction(Request $request)
    {
        ...
        $query = $em->getRepository('AdminBundle:Goods')->getGoodList();
...
//                    }
            ...
            if(!is_null($extra_fields) && count($extra_fields)){
                ...
                foreach($extra_fields as $field){
                    $extra_field = $em->getRepository('AdminBundle:Extra_fields')->find($field['id']);
     ...
     * @Template("AdminBundle:Good:show.html.twig")
     */
    ...
    public function getCategoryExtraField(Request $request){
        ...
        $set_of_catigories = $em->getRepository('AdminBundle:Category')->getParentsCategory($category_id);
        $good = null;
        if($good_id){
            //если мы изменяем существующий товар, ищем его
            $good = $em->getRepository('AdminBundle:Goods')->findOneBy(array('id' => $good_id));
            if(is_null($good)){
                throw new EntityNotFoundException("Такого товара не существет");
            }
        }
        ...
        foreach($set_of_catigories as $cat_id){
            $category = $em->getRepository('AdminBundle:Category')->find($cat_id['id']);
            ...
            if(count($extra_fs)){
                ...
                foreach($extra_fs as $f){
                    if(!$extra_fields->contains($f)){
                        if(!is_null($good)){
                            $gef = $em->getRepository('AdminBundle:GoodExtraField')->getExtraFiledValBayGoodIdAndFieldId($good->getId(), $f->getId());
                            if(!is_null($gef)) {
        ...
        return new JsonResponse(
            array('html' => $this->renderView(
                'AdminBundle:Good:extra_field_list.html.twig',
                array(
    ...
    public function getPhoto($good_id, $photo_name){
        $em = $this->getDoctrine()->getManager();
        $good_photo = $em->getRepository('AdminBundle:File')->findOneBy(array('good_id' => $good_id, 'name' => $photo_name));
    ...
    public function deleteAction(Request $request, $id)
    {
        ...
        if ($form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $entity = $em->getRepository('AdminBundle:Goods')->find($id);
