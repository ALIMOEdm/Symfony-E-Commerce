...
use AppBundle\Util\Files\PaginatorUtil;
...
class CatalogController extends Controller
{
    ...
    public function indexAction(Request $request){
        ...
        $entities = $em->getRepository('AppBundle:Category')->findBy(array('parent_category' => null ));
        foreach ($entities as &$entity) {
            $childs = $this->getDoctrine()->getManager()->getRepository('AppBundle:Category')->getFirstLineChildsCategory($entity->getId());
            if (!is_null($childs)) {
    ...
    public function goodsAction(Request $request, $id){
        ...
        // exit;
        $em = $this->getDoctrine()->getManager();
        $qty_arr = $em->getRepository('AppBundle:GoodExtraField')->getQtyGoodsForCatalog($id, $search, $brand, $extra_fields);
        $qty = $qty_arr["qty"];
        ...
        $goods = $em->getRepository('AppBundle:GoodExtraField')->getGoodsForCatalog($id, $search, $brand, $page, $per_page, $extra_fields, array("order_field" => $order, "sc" => $sc));
        return array(
            'id' => $id,
            'goods' => $goods,
    ...
    public function catalogAction(Request $request, $id){
        $em = $this->getDoctrine()->getManager();
        $bread_crumbs = $em->getRepository('AppBundle:Category')->getParentsCategory($id);
        $extra_fields_values = $em->getRepository('AppBundle:GoodExtraField')->getExtraFieldsValuesByCategoryId($id);
        ...
        $childs = $em->getRepository('AppBundle:Category')->getFirstLineChildsCategory($id);
        $brands = $em->getRepository('AppBundle:Category')->getBrandsForCategory($id);
        foreach ($childs as &$child) {
            $child["goods_qty"] = count($em->getRepository('AppBundle:Goods')->findBy(array('category' => $child["id"])));
        }
        unset($child);
    ...
    public function searchByNameAction($string){
        return new JsonResponse( $this->getDoctrine()->getManager()->getRepository('AppBundle:Goods')->getGoodsByLike($string) );
    }
